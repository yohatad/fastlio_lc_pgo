...
def generate_launch_description():
    ...
    declare_save_directory_cmd = DeclareLaunchArgument(
        'save_directory', default_value='/home/yoha/Lidar/run_indooroffice1_lc/pgo_output/',
    ...
    fast_lio_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            # IndoorOffice1_dataset's /ouster/points topic - 128-channel Ouster,
            # PointCloud2 with the standard ouster_ros::Point field layout.
            'config_file': 'ouster128_indoor.yaml',
            'rviz': rviz,
            'rviz_cfg': rviz_cfg,
        }.items()
    )
    ...
    pgo_node = Node(
        ...
        parameters=[{
            ...
            # keyframe selection - this run is only ~66s, use a tighter gap
            # than the corridor run so we still get enough keyframes
            'keyframe_meter_gap': 0.5,
            'keyframe_deg_gap': 10.0,

            # Scan Context (indoor office -> small radius)
            'sc_dist_thres': 0.4,
            'sc_max_radius': 20.0,
